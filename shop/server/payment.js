...
router.post('/create-payment', async (req, res) => {
  try {
    ...
    if (paymentMethod === 'cash') {
      ...
      return res.json({ orderId, redirectUrl: '/order-confirmation.html' });
    }
    ...
    const payment = await mollieClient.payments.create({
      ...
      redirectUrl: `${process.env.FRONTEND_URL}/order-confirmation.html`,
    ...
    orders[payment.metadata.orderId] = {
      status: 'pending',
    ...
    res.json({ orderId: payment.metadata.orderId, redirectUrl: payment.getCheckoutUrl() });
  } catch (error) {
...
router.post('/payment-webhook', async (req, res) => {
  try {
    ...
    const order = orders[payment.metadata.orderId];
    
    if (order) {
      order.status = payment.status;
      if (payment.isPaid()) {
        await sendConfirmationEmails(payment.metadata.orderId);
      }
    }
